        ...
        if self.dataset_attr.load_from in ["script", "file"]:
            if isinstance(medias[0], str):
                for i in range(len(medias)):
                    if ";base64" in medias[i]: # base64 encoded media
                        continue
                    else:
...
@dataclass
class SharegptDatasetConverterWithMask(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        tag_mapping = {
            self.dataset_attr.user_tag: Role.USER.value,
            self.dataset_attr.assistant_tag: Role.ASSISTANT.value,
        ...
        if example[self.dataset_attr.loss_mask] is not None and len(example[self.dataset_attr.loss_mask]) != len(aligned_messages):
            logger.warning_rank0(
                f"Loss mask length {len(example[self.dataset_attr.loss_mask])} does not match message count {len(aligned_messages)}."
            )
            broken_data = True
        ...
        if broken_data:
            ...
            raise ValueError("Loss mask is not supported in kto example.")
        elif (
            self.dataset_attr.ranking
            and isinstance(example[self.dataset_attr.chosen], dict)
            and isinstance(example[self.dataset_attr.rejected], dict)
        ):  # pairwise example
            raise ValueError("Loss mask is not supported in pairwise example.")
        else:  # normal example
            prompt = aligned_messages[:-1]
            response = aligned_messages[-1:]
        ...
        output = {
            ...
            "_videos": self._find_medias(example[self.dataset_attr.videos]) if self.dataset_attr.videos else None,
            "_audios": self._find_medias(example[self.dataset_attr.audios]) if self.dataset_attr.audios else None,
            "_loss_mask": example[self.dataset_attr.loss_mask] if self.dataset_attr.loss_mask else None,
        }
        return output
...
DATASET_CONVERTERS = {
    ...
    "sharegpt_mask": SharegptDatasetConverterWithMask,
}
...
def align_dataset(
    ...
    _videos: []
    _audios: []
    _loss_mask: [] (optional, only for sharegpt_mask dataset, default None)
    """
